		...
		<form action="{{ route("admin.tasks.update", $task) }}" method="POST">
			...
			<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
				<div class="mb-4">
					...
					<input type="number" name="user_id" id="user_id" value="{{ old("user_id", $task->user_id) }}"
				...
				<div class="mb-4">
					...
					<input type="number" step="0.01" name="amount" id="amount" value="{{ old("amount", $task->amount) }}"
						class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
						required>
				...
				<div class="mb-4">
					...
					<select name="urgency_level" id="urgency_level"
						class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
						required>
						<option value="urgent" {{ old("urgency_level", $task->urgency_level) == "urgent" ? "selected" : "" }}>Urgent
						</option>
						<option value="today" {{ old("urgency_level", $task->urgency_level) == "today" ? "selected" : "" }}>Today
						</option>
						<option value="tomorrow" {{ old("urgency_level", $task->urgency_level) == "tomorrow" ? "selected" : "" }}>Tomorrow
						</option>
						<option value="week" {{ old("urgency_level", $task->urgency_level) == "week" ? "selected" : "" }}>Week</option>
						<option value="custom" {{ old("urgency_level", $task->urgency_level) == "custom" ? "selected" : "" }}>Custom
						</option>
					</select>
				...
				<div class="mb-4">
					...
					<input type="text" name="address" id="address" value="{{ old("address", $task->address) }}"
						class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
				...
				<div class="mb-4">
					...
					<select name="contact_preference" id="contact_preference"
						...
						<option value="message"
							{{ old("contact_preference", $task->contact_preference) == "message" ? "selected" : "" }}>Message</option>
						<option value="call" {{ old("contact_preference", $task->contact_preference) == "call" ? "selected" : "" }}>
							Call</option>
						<option value="both" {{ old("contact_preference", $task->contact_preference) == "both" ? "selected" : "" }}>
							Both</option>
					</select>
				...
				<div class="mb-4">
					...
					<select name="privacy" id="privacy"
						class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
						required>
						<option value="public" {{ old("privacy", $task->privacy) == "public" ? "selected" : "" }}>Public</option>
						<option value="verified" {{ old("privacy", $task->privacy) == "verified" ? "selected" : "" }}>Verified</option>
						<option value="invite" {{ old("privacy", $task->privacy) == "invite" ? "selected" : "" }}>Invite</option>
					</select>
			...
			<div class="mb-4">
				...
				<textarea name="description" id="description" rows="4"
				 class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old("description", $task->description) }}</textarea>
			...
			<div class="mb-4">
				...
				<textarea name="additional_info" id="additional_info" rows="3"
				 class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old("additional_info", $task->additional_info) }}</textarea>
			...
			<div class="flex justify-end">
				...
				<button type="submit" class="rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">Update Task</button>
			</div>
		</form>
	</div>
@endsection
