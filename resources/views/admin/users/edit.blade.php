	...
	<form action="{{ route("admin.users.update", $user) }}" method="POST" enctype="multipart/form-data"
		...
		<div class="mb-4">
			...
			<textarea name="blocked_reason" id="blocked_reason" rows="3"
			 class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old("blocked_reason", $user->blocked_reason) }}</textarea>
		</div>

		<div class="flex justify-end">
			<a href="{{ route("admin.users.index") }}"
				class="mr-4 rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 hover:bg-gray-50">Cancel</a>
			<button type="submit" class="rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">Update User</button>
		</div>
	</form>
@endsection
