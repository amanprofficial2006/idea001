...
class TaskController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($task->status !== 'pending') {
            return response()->json([
                'success' => false,
                'message' => 'Task can only be edited while it is pending.',
            ], 422);
        }

        // Validation (similar to store, but fields are optional)
        $validator = Validator::make($request->all(), [
            ...
            'urgency_level' => 'sometimes|required|in:urgent,today,tomorrow,week',
            ...
            'contact_preference' => 'sometimes|required|in:message,call,both',
            'privacy' => 'sometimes|required|in:public,verified,invite',
        ...
        // Deadline logic if urgency_level is provided
        $finalDeadline = $task->deadline; // Keep existing if not changing
        ...
        try {
            // Update task
            $task->update(array_merge($validated, ['deadline' => $finalDeadline]));

            // Replace skills
            if (isset($validated['skills'])) {
                $task->skills()->delete(); // Delete old skills
                foreach ($validated['skills'] as $skill) {
                    if (!empty($skill)) {
                        TaskSkill::create([
                            'task_id' => $task->id,
            ...
            // Replace images
            if ($request->hasFile('images')) {
                $task->images()->delete(); // Delete old images (assuming soft delete or just delete)
                foreach ($request->file('images') as $image) {
                    $path = $image->store('tasks', 'public');
                    TaskImage::create([
                        'task_id' => $task->id,
